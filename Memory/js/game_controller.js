					...
					if (!this.current_card[i].done && this.current_card[i].texture !== back){
						if (front){
							if (front.texture === this.current_card[i].texture){
								...
								for (let j = 0; j < this.current_card.length; j++){
									console.log(this.current_card[j].texture)
									front = this.current_card[j].texture
								}
								Vue.set(this.current_card, i, {done: false, texture: back});
								Vue.set(this.current_card, i_front, {done: false, texture: back});
								this.bad_clicks++;
									//this.current_card[j].done = true;
								
								/*var tamany = this.current_card.length;
								console.log(tamany)
								setTimeout(function() {
									for (let j_i = 0; j_i < tamany; j_i++){
										Vue.set(this.current_card, j_i, {done: false, texture: back});
									}
								}, 10);*/
								break;
							}
